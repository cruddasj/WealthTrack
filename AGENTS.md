...
- The release pipeline automatically bumps `assets/version.json` to the next patch after a deploy. Do not edit this file manually. When preparing release notes, match the version number you document in the changelog files to the value in `assets/version.json` so the in-app updater stays in sync, even if that version has not shipped yet.

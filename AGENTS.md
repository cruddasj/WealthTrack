...
- You MUST maintain the changelog: always add a concise summary for non-technical audiences of user-facing changes to both `CHANGELOG.md` and `assets/changelog.json`.
- The release pipeline automatically bumps `assets/version.json` to the next patch for each PR (combining all changes in that PR into a single version bump). Do not edit this file manually. When preparing release notes, check the latest `v*` git tag. If `assets/version.json` matches the latest tag, base the next version on `assets/version.json` **plus one patch increment**. If `assets/version.json` is already ahead of the latest tag, base release notes on the **current `assets/version.json`**. Always timestamp changelog entries with the current date.

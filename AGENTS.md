...
- You MUST maintain the changelog: always add a concise summary for non-technical audiences of user-facing changes to both `CHANGELOG.md` and `assets/changelog.json`. Use the placeholders `[NEXT_VERSION]` and `[NEXT_DATE]` instead of hardcoding version numbers and dates in these files.
